...
copyBlob blobr = do
    ...
    bs   <- lift $ blobToByteString =<< resolveBlobRef (ByOid (Tagged oid))
    ...
    if not fastForward
        ...
        case refTarget ref of
            RefObj cr -> do
                ...
                cref <- ByOid . Tagged <$> parseOid (head commits1)
                mapM_ (flip copyCommit Nothing)
                    =<< mapM (\x -> ByOid . Tagged
                                    <$> (lift $ parseOid x))
                            (reverse commits1)
                case remoteHead of
                    ...
                    Just rh -> do
                        ...
                        if HashMap.member sha commits2
