...
runGit :: forall m a. MonadLgBase m
       => Git.GitT LgRepo (ReaderT LgRepo m) a -> m a
...
lgLookupReference :: MonadLg m => RefName -> m (Maybe RefTarget)
lgLookupReference = undefined

lgUpdateReference :: MonadLg m => RefName -> RefTarget -> m ()
lgUpdateReference = undefined

lgDeleteReference :: MonadLg m => RefName -> m ()
lgDeleteReference = undefined

lgAllReferences :: MonadLg m => m (Producer RefName m ())
lgAllReferences = undefined
...
lgParseOidIO :: String -> Int -> IO (Maybe Oid)
lgParseOidIO str len = do
    oid <- liftIO mallocForeignPtr
    r <- liftIO $ withCString str $ \cstr ->
...
lgParseOid :: MonadLg m => String -> m Oid
lgParseOid str
  | len > 40 = throwM (Git.OidParseFailed (pack str))
  | otherwise = do
      moid <- liftIO $ lgParseOidIO str len
      case moid of
          Nothing  -> throwM (Git.OidParseFailed (pack str))
          Just oid -> return oid
  where
    len = length str
